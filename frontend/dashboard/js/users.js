...
async function initUserManagement() {
    try {
        ...
        const authorRequestsResponse = await fetch("http://localhost:5501/api/author-registrations", {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        if (authorRequestsResponse.ok) {
            ...
            authorRequests = Array.isArray(requestsData.data) ? requestsData.data : requestsData;
        }
        
        // Đánh dấu user nào có đơn đăng ký
        usersData = users.map(user => ({
            ...user,
            hasAuthorRequest: authorRequests.some(request => 
...
function renderUserTable(users) {
    ...
    tbody.innerHTML = paginatedUsers.map((user, index) => `
        <tr data-id="${user.userid}">
            ...
            <td><span class="status-badge ${user.status || 'active'}">${getStatusName(user.status || 'active')}</span></td>
            <td>
                ...
                ${user.status === 'active' || !user.status
                    ? `<button class="btn btn-ban" data-id="${user.userid}" title="Khóa tài khoản">
                       ...
                       </button>` : `<button class="btn btn-unban" data-id="${user.userid}" title="Mở khóa">
                        <i class="fas fa-lock"></i>
                       </button>`}
                <button class="btn btn-delete" data-id="${user.userid}" title="Xóa">
...
function setupUserEventListeners() {
    ...
    document.querySelector('#users tbody')?.addEventListener('click', async (e) => {
        ...
        if (btn.classList.contains('btn-edit')) {
            ...
            try {
                const token = await getAccessTokenFromRefresh();
                const response = await fetch(`http://localhost:5501/api/author-registrations/user/${userId}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                const authorRequest = Array.isArray(data.data) ? data.data[0] : data.data;
                ...
                console.error(error);
                showToast(error.message || 'Lỗi khi tải đơn đăng ký tác giả', 'error');
            }
        } else if (btn.classList.contains('btn-revoke-author')) {
            const userId = parseInt(btn.getAttribute('data-id'));
            
            showConfirmModal('Bạn có chắc muốn thu hồi quyền tác giả của người dùng này?', async () => {
                try {
                    ...
                    console.error('Error:', error);
                    showToast(error.message || 'Có lỗi xảy ra khi thu hồi quyền tác giả', 'error');
                }
            });
        }    
    });
...
function fillAuthorRequestModal(data) {
    ...
    document.getElementById('authorExperience').textContent = data.experience || 'N/A';
    document.getElementById('authorPortfolio').innerHTML = data.portfolio ? `<a href="${data.portfolio}" target="_blank">Xem</a>` : 'Không có';
    document.getElementById('authorTopics').textContent = (data.topics || []).join(', ');
    ...
    newApproveBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5501/api/author-registrations/${authorRequestId}/status`, {
                method: 'PUT',
                headers: {
            ...
            const row = document.querySelector(`#authorRequestsBody tr[data-id="${authorRequestId}"]`);
            if (row) row.remove();
...
function setupSearchFunctionality() {
    ...
    if (searchButton) {
        searchButton.addEventListener('click', () => {
            const searchTerm = document.querySelector('#users .search-group input').value.trim().toLowerCase();
...
async function updateUserRole(userId, newRole) {
    try {
        ...
        const userResponse = await fetch(`http://localhost:5501/api/users/${userId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function submitUserForm() {
    ...
    try {
        ...
        if (currentEditingUserId) {
            ...
            response = await fetch(`http://localhost:5501/api/users/${currentEditingUserId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || 'Có lỗi khi thêm/cập nhật người dùng');
        }

        showToast(currentEditingUserId ? 'Cập nhật thành công' : 'Thêm quản trị viên thành công', 'success');
        closeModal();
        await initUserManagement();
    } catch (error) {
...
async function updateUserStatus(userId, status) {
    showConfirmModal(
        `Bạn có chắc muốn ${status === 'banned' ? 'khóa' : 'mở khóa'} người dùng này?`,
        async () => {
            try {
                const token = await getAccessTokenFromRefresh();
...
async function initAuthorRequests() {
    try {
      ...
      const requests = (Array.isArray(data.data) ? data.data : data).filter(r => r.status === 'pending');
  ...
  function renderAuthorRequests(requests) {
    ...
    document.querySelectorAll('.btn-view-author').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!userId || !requestData) return showToast('Không thể tìm dữ liệu đơn đăng ký', 'error');
