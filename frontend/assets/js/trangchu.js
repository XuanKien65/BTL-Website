...
async function loadPopularPosts() {
  try {
    ...
    const filteredPosts = data.data.posts.filter(post => !shownPostIds.has(post.postid)).slice(0, 5);
...
function renderTodayNews(posts) {
  ...
  if (!posts || posts.length < 3) {
    todayBigNews.innerHTML = '<div class="error-message">Không đủ bài viết để hiển thị layout.</div>';
    return;
  }
...
async function loadLeastViewedPosts() {
  try {
    ...
    // Lọc bài viết chưa hiển thị
    const leastViewedPosts = posts.filter(post => !shownPostIds.has(post.postid)).slice(0, 3);
    ...
    leastViewedPosts.forEach((post, index) => {
      const postHTML = `
        <a class="smallnews-content" href="/pages/trangbaiviet.html?slug=${post.slug}">
          <img class="smallnews-img" src="${post.featuredimage}" alt="${post.title}" />
...
async function loadTinKhac2(selector = '.tinkhac2 .news-container', requiredCount = 9) {
  try {
    const response = await fetch(`/api/posts?status=published`);
    ...
    posts.forEach(post => {
      ...
      article.innerHTML = `
        ...
        <div class="article-footer">
          ...
          <span class="article-date">${new Date(post.createdat).toLocaleDateString()}</span>
        </div>
      `;
...
async function fetchPostsByCategory(categoryName, container) {
  try {
    const response = await fetch(`/api/posts/search?categoryName=${encodeURIComponent(categoryName)}&status=published&pageSize=10`);
    const data = await response.json();
    
    // Lọc bài viết chưa hiển thị
    const newPosts = data.data.posts.filter(post => !shownPostIds.has(post.postid));
...
function renderTopicPosts(container, posts) {
  ...
  posts.forEach(post => {
    const postHTML = `
      <a class="grid__column-2" href="/pages/trangbaiviet.html?slug=${post.slug}" data-category="${post.categories[0]?.slug}">
        <div class="news-home-item">
          <div class="news-home-item--img" style="background-image: url(${post.featuredimage})"></div>
          <div class="news-home-item--content">
            ...
            <div class="news-home-item--meta">
              <span class="news-home-item--date">${new Date(post.createdat).toLocaleDateString('vi-VN')}</span>
              <span class="news-home-item--read-time">${post.views} lượt đọc</span>
            </div>
          </div>
        </div>
      </a>
...
function initTopicContainers() {
  document.querySelectorAll('.container[data-main-category]').forEach(container => {
    const sibling = container.nextElementSibling;
    const readMoreBtn = sibling && sibling.classList.contains('readmore-container') ? sibling : sibling?.querySelector('.readmore-container');
    ...
    container.querySelectorAll('.news-subject a').forEach(link => {
      link.addEventListener('click', async (e) => {
        ...
        container.querySelectorAll('.news-subject a').forEach(l => l.classList.remove('active'));
    ...
    if (readMoreBtn) {
      readMoreBtn.addEventListener('click', () => {
        const categorySlug = currentCategory || container.querySelector('.news-subject a')?.dataset.category;
        const categoryName = categoryMap[categorySlug] || 'Danh mục';
        window.location.href = `/pages/topic.html?categoryName=${encodeURIComponent(categoryName)}`;
      });
    }
...
async function loadRecommendedPosts() {
  try {
    const response = await fetch('/api/posts/search?sortBy=popular&pageSize=20&status=published');
    if (!response.ok) throw new Error('Lỗi tải dữ liệu');
    const data = await response.json();
    ...
    if (filteredPosts.length === 0) {
      document.querySelector('.news-slider-container').innerHTML += `<p class="error">Không có bài viết đề xuất mới</p>`;
      return;
    }
    ...
    swiperWrapper.innerHTML = filteredPosts.map(post => `
    ...
    document.querySelector('.news-slider-container').innerHTML += `<p class="error">Không thể tải đề xuất</p>`;
  }
}
...
async function loadSliderPosts(requiredCount = 6) {
  ...
  try {
    ...
    if (collectedPosts.length < requiredCount) {
      const response = await fetch(`/api/posts/search?sortBy=popular&pageSize=50&status=published`);
      const data = await response.json();

      const fallbackPosts = data.data.posts.filter(post => !tempShown.has(post.postid));
    ...
    collectedPosts.forEach((post, index) => {
      const slideHTML = `
        <a class="slide ${index === 0 ? 'active' : ''}" href="/pages/trangbaiviet.html?slug=${post.slug}">
          <img src="${post.featuredimage}" alt="${post.title}" />
          <div class="overlay">
            <h2>${post.title}</h2>
...
function initLoadMoreNews({
  ...
  function showErrorMessage() {
    document.querySelector(buttonSelector).innerHTML = '<div class="error-message">Không thể tải thêm bài viết</div>';
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initLoadMoreNews({
    ...
    renderItem: (post) => `
      <a class="highlighted-article" href="/pages/trangbaiviet.html?slug=${post.slug}">
        <img src="${post.featuredimage}" alt="${post.title}" />
