...
const authors = [
  {
    ...
    idFront:
      "https://via.placeholder.com/150x100?text=M%E1%BA%B7t+tr%C6%B0%E1%BB%9Bc",
  ...
  },
  {
    ...
    samples: ["https://example.com/bai-huong-dan-lam-dep"],
  },
  {
    id: 5,
    username: "author5",
...
function filterAuthors() {
  return authors.filter((a) => {
    const matchesSearch =
      a.username.toLowerCase().includes(currentFilter.searchTerm) ||
      a.email.toLowerCase().includes(currentFilter.searchTerm) ||
      a.phone.includes(currentFilter.searchTerm);

    const matchesField =
      currentFilter.field === "" || a.fields.includes(currentFilter.field);
...
function handleSearch() {
  currentFilter.searchTerm = document
...
function renderPagination() {
  ...
  if (currentPage > 1) {
    ...
    prevLi.innerHTML = `<a href="#" onclick="changePage(${
      currentPage - 1
    })">&laquo;</a>`;
